...
Specifically, we could draw the posterior samples from a fitted Bayesian conditional model. For each posterior sample, an estimate of
$\alpha_{lq}^{\mathrm{M}}$ could be obtained through post-hoc marginalization. 
Assuming $H$ samples are
drawn from posterior distribution in the Bayesian conditional model, we could
...
If we are also interested in the smooth functions, we can set `plot.smooth` as `TRUE` to plot the estimated smooth functions, 
```{r}
bmam.summary <- summary(bmam.fit, plot.smooth = TRUE)
```
